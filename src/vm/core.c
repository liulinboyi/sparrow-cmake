...
char *rootDir = NULL; //根目录 源码所在根目录其值是在cli.c里面设置的
...
//读取源代码文件
char *readFile(const char *path /*源码路径*/)
{
   FILE *file = fopen(path, "r"); // 获得源码句柄，为后面读取做准备
   if (file == NULL)
   {
   ...
   stat(path, &fileStat); // 获得源码大小
   size_t fileSize = fileStat.st_size;
   char *fileContent = (char *)malloc(fileSize + 1); // 为源码字符串分配内存，多申请一个是为了结尾的\0字符
   if (fileContent == NULL)
   {
      MEM_ERROR("Could`t allocate memory for reading file \"%s\".\n", path);
   }

   size_t numRead = fread(fileContent, sizeof(char), fileSize, file); // 获取源码串，到fileContnet
   if (numRead < fileSize)
   {
      IO_ERROR("Could`t read file \"%s\".\n", path);
   }
   fileContent[fileSize] = '\0'; // 人为在结尾加上\0字符，此时源码被读到fileContnet中
...
static bool primSystemClock(VM *vm UNUSED, Value *args UNUSED)
{
   ...
   RET_NUM((long)tv.tv_sec * 1000000 + tv.tv_usec) // 返回微妙
}
